...
# --- OpenCV Tileset Analysis Functions ---
def load_opencv_analysis():
    """Load the precise tileset analysis results from OpenCV."""
    try:
        with open('tileset_analysis.json', 'r') as f:
            return json.load(f)
    except (FileNotFoundError, json.JSONDecodeError):
        return None

def surfaces_match(surf1, surf2, tolerance=50):
    """Check if two surfaces are similar enough to be considered the same tile."""
    ...
    try:
        # Get average colors of both surfaces for quick comparison
        ...
        avg1 = get_average_color(surf1)
        avg2 = get_average_color(surf2)
        
        # Check if average colors are similar
        color_diff = abs(avg1[0] - avg2[0]) + abs(avg1[1] - avg2[1]) + abs(avg1[2] - avg2[2])
        
        # Consider it a match if the average colors are very similar
        return color_diff <= tolerance
        
    except Exception:
        return False

def score_floor_matching_corner(surface, corner_type, floor_tile):
    """Score how well a corner piece matches and complements the floor tile."""
    try:
        width, height = surface.get_size()
        
        # Get key colors from both tiles
        def get_dominant_colors(surf):
            colors = []
            w, h = surf.get_size()
            for x in range(0, w, max(1, w//4)):
                for y in range(0, h, max(1, h//4)):
                    try:
                        colors.append(surf.get_at((x, y))[:3])
                    except:
                        continue
            return colors
        
        surface_colors = get_dominant_colors(surface)
        floor_colors = get_dominant_colors(floor_tile)
        
        if not surface_colors or not floor_colors:
            return 0
        
        # Calculate color compatibility score
        compatibility_score = 0
        
        # Check if corner shares colors with floor (good for continuity)
        shared_colors = 0
        for surf_color in surface_colors:
            for floor_color in floor_colors:
                diff = abs(surf_color[0] - floor_color[0]) + abs(surf_color[1] - floor_color[1]) + abs(surf_color[2] - floor_color[2])
                if diff < 40:  # Similar colors
                    shared_colors += 1
                    break
        
        # Bonus for color compatibility
        if shared_colors > 0:
            compatibility_score += min(shared_colors * 2, 8)
        
        # Check for appropriate corner characteristics based on type
        corner_feature_score = 0
        center_x, center_y = width // 2, height // 2
        
        try:
            center_color = surface.get_at((center_x, center_y))[:3]
            
            # Sample strategic points based on corner type
            if corner_type == 'inner_top_left':
                # For top-left, expect variation from top-left toward bottom-right
                sample_points = [(width//4, height//4), (3*width//4, 3*height//4)]
            elif corner_type == 'inner_top_right':
                # For top-right, expect variation from top-right toward bottom-left
                sample_points = [(3*width//4, height//4), (width//4, 3*height//4)]
            elif corner_type == 'inner_bottom_left':
                # For bottom-left, expect variation from bottom-left toward top-right
                sample_points = [(width//4, 3*height//4), (3*width//4, height//4)]
            elif corner_type == 'inner_bottom_right':
                # For bottom-right, expect variation from bottom-right toward top-left
                sample_points = [(3*width//4, 3*height//4), (width//4, height//4)]
            else:
                sample_points = [(width//4, height//4), (3*width//4, 3*height//4)]
            
            # Check for smooth transitions (good corner characteristic)
            color_variations = 0
            for x, y in sample_points:
                if 0 <= x < width and 0 <= y < height:
                    sample_color = surface.get_at((x, y))[:3]
                    diff = abs(center_color[0] - sample_color[0]) + abs(center_color[1] - sample_color[1]) + abs(center_color[2] - sample_color[2])
                    if 10 < diff < 80:  # Some variation but not too extreme
                        color_variations += 1
            
            if color_variations > 0:
                corner_feature_score += 3
                
        except:
            pass
        
        # Check for reasonable brightness (not too dark or too bright)
        avg_brightness = sum(sum(color) for color in surface_colors) / (len(surface_colors) * 3) if surface_colors else 128
        brightness_score = 0
        if 40 <= avg_brightness <= 200:
            brightness_score = 2
        elif avg_brightness < 20 or avg_brightness > 240:
            brightness_score = -2
        
        # Bonus for texture detail (avoid solid colors)
        unique_colors = len(set(surface_colors))
        detail_score = min(unique_colors // 2, 3) if unique_colors > 2 else -1
        
        total_score = compatibility_score + corner_feature_score + brightness_score + detail_score
        return max(0, total_score)
        
    except Exception:
        return 0

def extract_corners_from_room_backgrounds(dungeon_level):
    """
    Extract corner pieces from authentic Undertale room backgrounds like bg_firstroom.png.
    This gives us the natural, rounded corners that Undertale uses in actual rooms.
    """
    try:
        # Map dungeon levels to appropriate Undertale room backgrounds
        room_backgrounds = {
            1: "assets/undertale/Overworld/Locations/01 - Ruins/Backgrounds/bg_firstroom.png",  # Ruins
            2: "assets/undertale/Overworld/Locations/02 - Snowdin/Backgrounds/bg_snowdin.png",  # Snowdin (if exists)
            3: "assets/undertale/Overworld/Locations/03 - Waterfall/Backgrounds/bg_waterfall.png",  # Waterfall (if exists)
            4: "assets/undertale/Overworld/Locations/04 - Hotland/Backgrounds/bg_hotland.png",  # Hotland (if exists)
            5: "assets/undertale/Overworld/Locations/05 - Core/Backgrounds/bg_core.png"  # Core (if exists)
        }
        
        # Try to find a room background for this level
        room_path = room_backgrounds.get(dungeon_level)
        if not room_path or not os.path.exists(room_path):
            # Fallback to bg_firstroom.png for all levels if specific ones don't exist
            room_path = "assets/undertale/Overworld/Locations/01 - Ruins/Backgrounds/bg_firstroom.png"
            if not os.path.exists(room_path):
                return None
        
        print(f"        Analyzing room background: {os.path.basename(room_path)}")
        room_image = pygame.image.load(room_path)
        room_width, room_height = room_image.get_size()
        
        # bg_firstroom.png is 680x260 - analyze it to find natural corner patterns
        corner_tiles = {}
        tile_size = TILE_SIZE
        
        # Define where to extract corner samples from the room background
        # Based on typical Undertale room layout patterns
        corner_sample_regions = {
            # Inner corners (most important - these create the rounded room feel)
            'inner_top_left': (50, 40, 60, 60),      # Top-left area where walls meet floor
            'inner_top_right': (room_width-110, 40, 60, 60),  # Top-right area
            'inner_bottom_left': (50, room_height-100, 60, 60),  # Bottom-left area  
            'inner_bottom_right': (room_width-110, room_height-100, 60, 60),  # Bottom-right area
            
            # Outer corners (for more complex room shapes)
            'outer_top_left': (80, 70, 40, 40),
            'outer_top_right': (room_width-120, 70, 40, 40),
            'outer_bottom_left': (80, room_height-110, 40, 40),
            'outer_bottom_right': (room_width-120, room_height-110, 40, 40)
        }
        
        for corner_type, (x, y, w, h) in corner_sample_regions.items():
            try:
                # Extract the corner region from the room background
                if x + w <= room_width and y + h <= room_height:
                    corner_rect = pygame.Rect(x, y, w, h)
                    corner_surface = pygame.Surface((w, h))
                    corner_surface.blit(room_image, (0, 0), corner_rect)
                    
                    # Scale to tile size and store
                    scaled_corner = pygame.transform.scale(corner_surface, (tile_size, tile_size))
                    corner_tiles[corner_type] = scaled_corner
                    print(f"        ✓ Extracted {corner_type} from room background at ({x}, {y})")
                    
            except Exception as e:
                print(f"        ✗ Failed to extract {corner_type}: {e}")
                continue
        
        return corner_tiles if corner_tiles else None
        
    except Exception as e:
        print(f"        ✗ Room background analysis failed: {e}")
        return None

def extract_tiles_from_tileset(tileset_path, tile_size=TILE_SIZE):
    """Extract floor and wall tiles from an Undertale tileset using OpenCV analysis."""
    try:
        ...
        # Load OpenCV analysis results for precise extraction
        opencv_analysis = load_opencv_analysis()
        tileset_name = os.path.basename(tileset_path)
        analysis_data = opencv_analysis.get(tileset_name) if opencv_analysis else None
        
        # Define better tile extraction positions based on common Undertale tileset layouts
        floor_tile = None
        wall_tile = None
        
        # Use OpenCV analysis results if available
        if analysis_data and analysis_data.get('floor_tile'):
            floor_x, floor_y = analysis_data['floor_tile']
            print(f"    Using OpenCV-analyzed floor tile at ({floor_x}, {floor_y})")
            
            try:
                x = floor_x * undertale_tile_size
                y = floor_y * undertale_tile_size
                ...
                print(f"      ✓ Floor tile extracted using OpenCV analysis")
            except (pygame.error, ValueError) as e:
                print(f"      ✗ OpenCV floor extraction failed: {e}")
                floor_tile = None
        
        # Extract wall tile using OpenCV analysis
        if analysis_data and analysis_data.get('wall_tile'):
            wall_x, wall_y = analysis_data['wall_tile']
            print(f"    Using OpenCV-analyzed wall tile at ({wall_x}, {wall_y})")
            
            try:
                x = wall_x * undertale_tile_size
                y = wall_y * undertale_tile_size
                ...
                print(f"      ✓ Wall tile extracted using OpenCV analysis")
            except (pygame.error, ValueError) as e:
                print(f"      ✗ OpenCV wall extraction failed: {e}")
                wall_tile = None
        
        # Fallback to manual positions if OpenCV analysis not available
        if not floor_tile:
            ...
            # Try to extract floor tile using fallback positions
        ...
        # Fallback wall tile extraction if OpenCV analysis not available
        if not wall_tile:
            # Try to find wall tiles - usually solid-looking tiles with borders
            if "ruinseasynam" in tileset_path:
                # For Ruins: stone/brick walls are typically in specific positions
                wall_positions = [
                    (4, 0), (5, 0), (6, 0), (7, 0),  # Right side of top row
                    (4, 1), (5, 1), (6, 1), (7, 1),  # Right side of second row  
                    (0, 2), (1, 2), (2, 2), (3, 2)   # Third row
                ]
            elif "tundratiles" in tileset_path:
                # For Snowdin: ice/snow walls
                wall_positions = [
                    (4, 0), (5, 0), (6, 0), (7, 0),  # Snow/ice wall tiles
                    (0, 2), (1, 2), (2, 2), (3, 2),  # More wall options
                    (4, 1), (5, 1), (6, 1)           # Additional ice walls
                ]
            elif "watertiles" in tileset_path:
                # For Waterfall: cave/rock walls
                wall_positions = [
                    (0, 0), (1, 0), (6, 0), (7, 0),  # Cave wall tiles
                    (8, 0), (9, 0), (10, 0),         # More cave walls
                    (0, 2), (5, 2), (6, 2)           # Rock formations
                ]
            elif "firetiles" in tileset_path or "labtiles" in tileset_path:
                # For Hotland: metal/lava walls  
                wall_positions = [
                    (0, 0), (1, 0), (5, 0), (6, 0),  # Metal/industrial walls
                    (7, 0), (8, 0), (9, 0),          # More metal tiles
                    (0, 4), (1, 4), (2, 4)           # Lava/fire walls
                ]
            else:
                # Generic wall positions
                wall_positions = [(1, 1), (2, 1), (0, 2), (1, 2)]
            
            # Try to extract wall tile using positions  
        ...
        # Try to extract corner tiles using OpenCV analysis first
        corner_tiles = {}
        wall_pieces = {}
        
        print(f"      Extracting corner pieces with OpenCV precision...")
        
        # For Undertale rounded rooms (like bg_firstroom.png), we need both inner and outer corners
        # Inner corners create the main room curves, outer corners add natural transitions
        corner_types = ['inner_top_left', 'inner_top_right', 'inner_bottom_left', 'inner_bottom_right']
        
        # Use OpenCV analysis results for precise corner extraction
        if analysis_data and analysis_data.get('corners'):
            opencv_corners = analysis_data['corners']
            print(f"      Using OpenCV-analyzed corners from {tileset_name}")
            
            for corner_type in corner_types:
                if corner_type in opencv_corners:
                    tx, ty = opencv_corners[corner_type]
                    
                    if 0 <= tx < tiles_per_row and 0 <= ty < tiles_per_col:
                        try:
                            x = tx * undertale_tile_size
                            y = ty * undertale_tile_size
                            corner_rect = pygame.Rect(x, y, undertale_tile_size, undertale_tile_size)
                            corner_surface = tileset_image.subsurface(corner_rect).copy()
                            corner_surface = pygame.transform.scale(corner_surface, (tile_size, tile_size))
                            corner_tiles[corner_type] = corner_surface
                            print(f"        ✓ {corner_type} extracted at OpenCV position ({tx}, {ty})")
                        except (pygame.error, ValueError) as e:
                            print(f"        ✗ OpenCV corner {corner_type} extraction failed: {e}")
        
        # Extract wall edge pieces for puzzle-piece system
        print(f"      Extracting wall edge pieces...")
        
        # For now, use the wall_tile for all wall pieces, but we can enhance this later
        if wall_tile:
            # Create wall pieces - for now they're all the same wall texture
            # In a more advanced system, we'd extract specific edge and corner pieces from the tileset
            wall_pieces = {
                "corner_inner_top_left": wall_tile.copy(),
                "corner_inner_top_right": wall_tile.copy(),
                "corner_inner_bottom_left": wall_tile.copy(), 
                "corner_inner_bottom_right": wall_tile.copy(),
                "corner_outer_top_left": wall_tile.copy(),
                "corner_outer_top_right": wall_tile.copy(),
                "corner_outer_bottom_left": wall_tile.copy(),
                "corner_outer_bottom_right": wall_tile.copy(),
                "edge_horizontal_top": wall_tile.copy(),
                "edge_horizontal_bottom": wall_tile.copy(),
                "edge_vertical_left": wall_tile.copy(),
                "edge_vertical_right": wall_tile.copy(),
                "edge_top": wall_tile.copy(),
                "edge_bottom": wall_tile.copy(),
                "edge_left": wall_tile.copy(),
                "edge_right": wall_tile.copy(),
                "wall_solid": wall_tile.copy()
            }
            print(f"        ✓ Generated {len(wall_pieces)} wall piece types from base wall texture")
...
def load_sprites():
    """Load all sprite images from Dungeon Crawl assets."""
    global sprites
    
    print("=== Loading Dungeon Crawl sprites ===")
    
    # Basic tile sprites from dungeon folder
    sprite_files = {
        # Floor tiles from crawl-tiles
        "floor_sandstone_floor0.png": "assets/crawl-tiles Oct-5-2010/dc-dngn/floor/sandstone_floor0.png",
        "floor_dirt0.png": "assets/crawl-tiles Oct-5-2010/dc-dngn/floor/dirt0.png", 
        "floor_pebble_brown0.png": "assets/crawl-tiles Oct-5-2010/dc-dngn/floor/pebble_brown0.png",
        "floor_marble_floor1.png": "assets/crawl-tiles Oct-5-2010/dc-dngn/floor/marble_floor1.png",
        "floor_stone_floor0.png": "assets/crawl-tiles Oct-5-2010/dc-dngn/floor/sandstone_floor0.png",
        "floor_wooden_floor.png": "assets/crawl-tiles Oct-5-2010/dc-dngn/floor/pebble_brown0.png",
        
        # Wall tiles from crawl-tiles  
        "wall_stone_brick1.png": "assets/crawl-tiles Oct-5-2010/dc-dngn/wall/stone_brick1.png",
        "wall_stone_dark0.png": "assets/crawl-tiles Oct-5-2010/dc-dngn/wall/stone_dark0.png",
        "wall_brick_brown0.png": "assets/crawl-tiles Oct-5-2010/dc-dngn/wall/brick_brown0.png", 
        "wall_marble_wall1.png": "assets/crawl-tiles Oct-5-2010/dc-dngn/wall/marble_wall1.png",
        "wall_sandstone_wall0.png": "assets/crawl-tiles Oct-5-2010/dc-dngn/wall/sandstone_wall0.png",
        "wall_metal_wall_brown.png": "assets/crawl-tiles Oct-5-2010/dc-dngn/wall/dngn_metal_wall_brown.png",
    }
    ...
    for sprite_name, file_path in sprite_files.items():
        if os.path.exists(file_path):
            try:
                sprites[sprite_name] = pygame.image.load(file_path)
                print(f"  Loaded: {sprite_name}")
            except pygame.error as e:
                print(f"  Error loading {sprite_name}: {e}")
                # Create a simple colored rectangle as fallback
                sprites[sprite_name] = pygame.Surface((TILE_SIZE, TILE_SIZE))
                if "floor" in sprite_name:
                    sprites[sprite_name].fill((139, 119, 101))  # Brown floor
                else:
                    sprites[sprite_name].fill((105, 105, 105))  # Gray wall
        else:
...
def extract_corners_from_room_backgrounds(dungeon_level):
    ...
    try:
        ...
        """Legacy scoring function - fallback for when floor matching fails."""
        try:
            width, height = surface.get_size()
            
            # Get average colors for comparison
            def get_color_at_safe(surf, x, y):
                if 0 <= x < surf.get_width() and 0 <= y < surf.get_height():
                    return surf.get_at((x, y))[:3]  # RGB only
                return (128, 128, 128)  # Gray fallback
            
            # Sample key positions
            center_x, center_y = width // 2, height // 2
            center_color = get_color_at_safe(surface, center_x, center_y)
            
            # Get floor reference color
            floor_color = get_color_at_safe(floor_ref, floor_ref.get_width()//2, floor_ref.get_height()//2)
            
            # Calculate how floor-like this tile is
            color_diff = abs(center_color[0] - floor_color[0]) + abs(center_color[1] - floor_color[1]) + abs(center_color[2] - floor_color[2])
            floor_similarity = max(0, (255 - color_diff) / 255.0) * 5
            
            # Check for texture variety
            colors_seen = [center_color]
            for x in range(0, width, width//4):
                for y in range(0, height, height//4):
                    colors_seen.append(get_color_at_safe(surface, x, y))
            
            unique_colors = len(set(colors_seen))
            texture_score = min(unique_colors // 2, 3)
            
            return max(0.0, floor_similarity + texture_score)
            
        except Exception:
            return 0.0
        """Score a tile based on how well it matches authentic Undertale corner pieces."""
        try:
            width, height = surface.get_size()
            ...
            # Sample key positions for Undertale corner analysis
            center_x, center_y = width // 2, height // 2
            center_color = get_color_at_safe(surface, center_x, center_y)
            
            # Get floor reference color (center of floor tile)  
            floor_color = get_color_at_safe(floor_ref, floor_ref.get_width()//2, floor_ref.get_height()//2)
            
            # Calculate how floor-like this tile is
            color_diff = abs(center_color[0] - floor_color[0]) + abs(center_color[1] - floor_color[1]) + abs(center_color[2] - floor_color[2])
            floor_similarity = max(0, (255 - color_diff) / 255.0) * 5  # Scale 0-5
            
            # Check for authentic Undertale corner characteristics
            corner_score = 0
            
            # Sample strategic points for each corner type
            if corner_type == 'inner_top_left':
                # Top-left inner corner - check bottom-right quadrant for curves
                sample_points = [(width*3//4, height*3//4), (width//2, height*3//4), (width*3//4, height//2)]
                
            elif corner_type == 'inner_top_right':
                # Top-right inner corner - check bottom-left quadrant for curves  
                sample_points = [(width//4, height*3//4), (width//2, height*3//4), (width//4, height//2)]
                
            elif corner_type == 'inner_bottom_left':
                # Bottom-left inner corner - check top-right quadrant for curves
                sample_points = [(width*3//4, height//4), (width//2, height//4), (width*3//4, height//2)]
                
            elif corner_type == 'inner_bottom_right':
                # Bottom-right inner corner - check top-left quadrant for curves
                sample_points = [(width//4, height//4), (width//2, height//4), (width//4, height//2)]
            else:
                sample_points = [(width//4, height//4), (width*3//4, height*3//4)]  # Generic
            
            # Check for color variety and curves at sample points
            colors_seen = [center_color]
            for x, y in sample_points:
                sample_color = get_color_at_safe(surface, x, y)
                colors_seen.append(sample_color)
                
                # Check if this sample differs from center (indicates curves/detail)
                sample_diff = abs(sample_color[0] - center_color[0]) + abs(sample_color[1] - center_color[1]) + abs(sample_color[2] - center_color[2])
                if sample_diff > 15:  # Some color variation
                    corner_score += 1
            
            # Count unique colors (indicates texture detail)
            unique_colors = 0
            for i, color1 in enumerate(colors_seen):
                is_unique = True
                for j, color2 in enumerate(colors_seen[:i]):
                    diff = abs(color1[0] - color2[0]) + abs(color1[1] - color2[1]) + abs(color1[2] - color2[2])
                    if diff < 20:  # Similar enough to count as same
                        is_unique = False
                        break
                if is_unique:
                    unique_colors += 1
            
            texture_score = min(unique_colors, 4)  # Cap at 4 points
            
            # Avoid completely uniform tiles (not authentic Undertale style)
            if unique_colors <= 1:
                texture_score -= 2
                
            # Check brightness - avoid too dark or too bright tiles  
            brightness = (center_color[0] + center_color[1] + center_color[2]) / 3
            brightness_score = 0
            if 40 <= brightness <= 200:  # Good range for floor tiles
                brightness_score = 1
            elif brightness < 20 or brightness > 240:  # Too extreme
                brightness_score = -2
                
            total_score = floor_similarity + corner_score + texture_score + brightness_score
...
def load_sprites():
    ...
    for level, tileset_config in level_tilesets.items():
        ...
        print(f"    Checking primary tileset: {primary_path}")
        print(f"    Exists: {os.path.exists(primary_path)}")
        ...
        if os.path.exists(primary_path):
            try:
                ...
                if floor_tile and wall_tile:
                    ...
                    print(f"    ✓ Primary tileset loaded successfully: {tileset_loaded}")
                else:
                    print(f"    ✗ Primary tileset failed to extract tiles")
            except Exception as e:
                print(f"    ✗ Error loading primary tileset: {e}")
        else:
            print(f"    ✗ Primary tileset file not found")
        
        # Try backup tilesets if primary failed
        if not (floor_tile and wall_tile) and "backup_tilesets" in tileset_config:
            print(f"    Trying backup tilesets...")
            for backup_tileset in tileset_config["backup_tilesets"]:
                backup_path = os.path.join(tileset_config["folder"], backup_tileset)
                print(f"    Checking backup: {backup_path} (exists: {os.path.exists(backup_path)})")
                if os.path.exists(backup_path):
                    try:
                        floor_tile, wall_tile, corner_tiles = extract_tiles_from_tileset(backup_path)
                        if floor_tile and wall_tile:
                            tileset_loaded = backup_tileset
                            print(f"    ✓ Backup tileset loaded successfully: {tileset_loaded}")
                            break
                        else:
                            print(f"    ✗ Backup tileset failed to extract tiles")
                    except Exception as e:
                        print(f"    ✗ Error loading backup tileset: {e}")
        ...
        if not floor_tile:
            ...
            if level == 1:  # Ruins - lighter pink for interior floors
                floor_tile.fill((200, 170, 190))  # Lighter pink blocks for inside
        ...
        if not wall_tile:
            ...
            if level == 1:  # Ruins - darker, more purple walls
                wall_tile.fill((70, 45, 70))      # Darker purple walls
    ...
    # Add wall pieces for puzzle-piece system to each level
    for level in level_sprites:
        wall_tile = level_sprites[level]["wall"]
        tileset_name = level_sprites[level]["tileset_name"]
        
        if wall_tile and tileset_name != "fallback":
            # Extract authentic wall pieces from the tileset using OpenCV analysis
            wall_pieces = extract_wall_pieces_from_tileset(level, tileset_name, level_sprites[level])
            if wall_pieces:
                level_sprites[level]["walls"] = wall_pieces
                print(f"    Added {len(wall_pieces)} authentic wall pieces for Level {level}")
            else:
                # Fallback to simple copying if extraction fails
                wall_pieces = create_fallback_wall_pieces(wall_tile)
                level_sprites[level]["walls"] = wall_pieces
                print(f"    Added {len(wall_pieces)} fallback wall pieces for Level {level}")
        elif wall_tile:
            # Use fallback system for levels without proper tilesets
            wall_pieces = create_fallback_wall_pieces(wall_tile)
            level_sprites[level]["walls"] = wall_pieces
            print(f"    Added {len(wall_pieces)} fallback wall pieces for Level {level}")
...
def create_fallback_wall_pieces(wall_tile):
    """Create fallback wall pieces by copying the base wall tile."""
    return {
        "corner_inner_top_left": wall_tile.copy(),
        "corner_inner_top_right": wall_tile.copy(),
        "corner_inner_bottom_left": wall_tile.copy(), 
...
def extract_wall_pieces_from_tileset(level, tileset_name, level_data):
    """Extract different wall pieces from the actual tileset using OpenCV analysis."""
    try:
        # Load OpenCV analysis data
        analysis_data = load_opencv_analysis_data()
        if not analysis_data or tileset_name not in analysis_data:
            print(f"      No OpenCV analysis found for {tileset_name}, using fallback")
            return None
        
        tileset_info = analysis_data[tileset_name]
        
        # Determine tileset path based on level
        level_tilesets = {
            1: "assets/undertale/Overworld/Locations/01 - Ruins/Tilesets/bg_ruinseasynam1.png",
            2: "assets/undertale/Overworld/Locations/02 - Snowdin/Tilesets/bg_tundratiles.png", 
            3: "assets/undertale/Overworld/Locations/03 - Waterfall/Tilesets/bg_watertiles.png",
            4: "assets/undertale/Overworld/Locations/04 - Hotland/Tilesets/bg_firetiles.png",
            5: "assets/undertale/Overworld/Locations/04 - Hotland/Tilesets/bg_labtiles.png"
        }
        
        tileset_path = level_tilesets.get(level)
        if not tileset_path or not os.path.exists(tileset_path):
        ...
        # Extract corner pieces using OpenCV coordinates
        corners = tileset_info.get("corners", {})
        for corner_type, (tx, ty) in corners.items():
            try:
                x = tx * undertale_tile_size
                y = ty * undertale_tile_size
                corner_rect = pygame.Rect(x, y, undertale_tile_size, undertale_tile_size)
                corner_surface = tileset_image.subsurface(corner_rect).copy()
                corner_surface = pygame.transform.scale(corner_surface, (tile_size, tile_size))
                wall_pieces[f"corner_{corner_type}"] = corner_surface
                print(f"      ✓ Extracted authentic {corner_type} from tileset at ({tx}, {ty})")
            except (pygame.error, ValueError) as e:
                print(f"      ✗ Failed to extract {corner_type}: {e}")
        
        # Get base wall tile for edges and solid walls
        wall_coord = tileset_info.get("wall_tile", [4, 0])
        tx, ty = wall_coord
        base_wall_surface = None
        
        try:
            x = tx * undertale_tile_size
            y = ty * undertale_tile_size
            wall_rect = pygame.Rect(x, y, undertale_tile_size, undertale_tile_size)
            base_wall_surface = tileset_image.subsurface(wall_rect).copy()
            base_wall_surface = pygame.transform.scale(base_wall_surface, (tile_size, tile_size))
        except (pygame.error, ValueError):
            base_wall_surface = level_data["wall"]  # Fallback
        
        # For edge pieces, try to find specific edge tiles or use transformed base wall
        edge_pieces = [
            "edge_horizontal_top", "edge_horizontal_bottom", 
            "edge_vertical_left", "edge_vertical_right",
            "edge_top", "edge_bottom", "edge_left", "edge_right"
        ]
        
        # Look for edge tiles in the tileset (common patterns around the base wall)
        wall_tx, wall_ty = wall_coord
        edge_search_positions = [
            (wall_tx-1, wall_ty), (wall_tx+1, wall_ty),  # Left/right of wall
            (wall_tx, wall_ty-1), (wall_tx, wall_ty+1),  # Above/below wall
            (wall_tx-1, wall_ty-1), (wall_tx+1, wall_ty-1), # Corners
            (wall_tx-1, wall_ty+1), (wall_tx+1, wall_ty+1)
        ]
        
        extracted_edges = []
        tileset_width = tileset_info["dimensions"][0]
        tileset_height = tileset_info["dimensions"][1]
        
        for edge_tx, edge_ty in edge_search_positions:
            if 0 <= edge_tx < tileset_width and 0 <= edge_ty < tileset_height:
                try:
                    x = edge_tx * undertale_tile_size
                    y = edge_ty * undertale_tile_size
                    edge_rect = pygame.Rect(x, y, undertale_tile_size, undertale_tile_size)
                    edge_surface = tileset_image.subsurface(edge_rect).copy()
                    edge_surface = pygame.transform.scale(edge_surface, (tile_size, tile_size))
                    extracted_edges.append(edge_surface)
                except (pygame.error, ValueError):
                    continue
        
        # Assign extracted edges to edge types, or use base wall as fallback
        for i, edge_type in enumerate(edge_pieces):
            if i < len(extracted_edges):
                wall_pieces[edge_type] = extracted_edges[i]
            else:
                wall_pieces[edge_type] = base_wall_surface.copy()
        
        # Add solid wall piece
        wall_pieces["wall_solid"] = base_wall_surface.copy()
        
        # Add outer corner pieces (transformed inner corners or base wall)
        outer_corner_types = [
            "corner_outer_top_left", "corner_outer_top_right", 
            "corner_outer_bottom_left", "corner_outer_bottom_right"
        ]
        for outer_type in outer_corner_types:
            # Try to use a variant or just use base wall
            wall_pieces[outer_type] = base_wall_surface.copy()
        
        print(f"      ✓ Extracted {len(wall_pieces)} wall pieces from authentic tileset")
        return wall_pieces
        
    except Exception as e:
        print(f"      ✗ Error extracting wall pieces from tileset: {e}")
        return None

def load_opencv_analysis_data():
    """Load the precise tileset analysis results from OpenCV."""
    try:
        with open('tileset_analysis.json', 'r') as f:
            return json.load(f)
    ...
    def extract_tiles_from_tileset(tileset_path, tile_size=TILE_SIZE):
        ...
        try:
            ...
            if not wall_tile:
                # Set wall positions based on tileset type (fallback method)
                if "ruinseasynam" in tileset_path:
                    wall_positions = [(4, 0), (5, 0), (6, 0), (7, 0), (3, 1), (4, 1), (5, 1)]
                elif "tundratiles" in tileset_path:
                    wall_positions = [(4, 0), (5, 0), (6, 0), (7, 0), (0, 2), (1, 2), (2, 2), (3, 2)]
                elif "watertiles" in tileset_path:
                    wall_positions = [(0, 0), (1, 0), (6, 0), (7, 0), (0, 1), (1, 1), (6, 1), (7, 1), (5, 2), (6, 2), (7, 2)]
                elif "firetiles" in tileset_path or "labtiles" in tileset_path:
                    wall_positions = [(5, 0), (6, 0), (7, 0), (8, 0), (4, 1), (5, 1), (6, 1), (7, 1), (0, 4), (1, 4), (2, 4)]
                else:
                    wall_positions = [(3, 0), (4, 0), (1, 1), (2, 1)]
            ...
            if analysis_data and analysis_data.get('corners'):
                ...
                # Also try to extract outer corners if we have good inner corners
                if len(corner_tiles) >= 2:
                    outer_types = ['outer_top_left', 'outer_top_right', 'outer_bottom_left', 'outer_bottom_right']
                    for outer_type in outer_types:
                        # Use variations of inner corner positions for outer corners
                        base_type = outer_type.replace('outer_', 'inner_')
                        if base_type in opencv_corners:
                            tx, ty = opencv_corners[base_type]
                            
                            # Try adjacent positions for outer corners
                            for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                                alt_x, alt_y = tx + dx, ty + dy
                                if 0 <= alt_x < tiles_per_row and 0 <= alt_y < tiles_per_col:
                                    try:
                                        x = alt_x * undertale_tile_size
                                        y = alt_y * undertale_tile_size
                                        corner_rect = pygame.Rect(x, y, undertale_tile_size, undertale_tile_size)
                                        corner_surface = tileset_image.subsurface(corner_rect).copy()
                                        corner_surface = pygame.transform.scale(corner_surface, (tile_size, tile_size))
                                        corner_tiles[outer_type] = corner_surface
                                        print(f"        ✓ {outer_type} extracted at adjacent position ({alt_x}, {alt_y})")
                                        break
                                    except (pygame.error, ValueError):
                                        continue
            
            # Temporarily disable room background extraction and use tileset method
            # TODO: Re-enable room background extraction after fixing encoding issues
            corners_from_rooms = None
            if corners_from_rooms:
                print(f"      ✓ Using authentic room background corners for level {self.dungeon_level}")
                corner_tiles.update(corners_from_rooms)
            else:
                print(f"      Using tileset-based corner extraction for level {self.dungeon_level}")
                
                # Continue with the existing tileset-based extraction...            # Find the floor tile first to get its exact position and characteristics
            floor_tile_pos = None
            
            # Get the floor tile position that we're using for the main floor
            if floor_tile:
                # Find where this floor tile came from in the tileset
                for test_y in range(tiles_per_col):
                    for test_x in range(tiles_per_row):
                        try:
                            x = test_x * undertale_tile_size
                            y = test_y * undertale_tile_size
                            test_rect = pygame.Rect(x, y, undertale_tile_size, undertale_tile_size)
                            
                            test_surface = pygame.Surface((undertale_tile_size, undertale_tile_size))
                            test_surface.blit(tileset_image, (0, 0), test_rect)
                            
                            # Compare with our current floor tile
                            if surfaces_match(test_surface, floor_tile):
                                floor_tile_pos = (test_x, test_y)
                                print(f"      Floor tile found at position ({test_x}, {test_y})")
                                break
                        except:
                            continue
                    if floor_tile_pos:
            ...
            # Now find corner pieces based on the floor tile position and tileset layout
            if floor_tile_pos:
                floor_x, floor_y = floor_tile_pos
                
                # Define where to look for corner pieces relative to the floor tile
                # This is based on typical Undertale tileset organization patterns
                corner_search_offsets = {
                    'inner_top_left': [(-1, -1), (-1, 0), (0, -1), (-2, -1), (-1, -2)],
                    'inner_top_right': [(1, -1), (1, 0), (0, -1), (2, -1), (1, -2)],
                    'inner_bottom_left': [(-1, 1), (-1, 0), (0, 1), (-2, 1), (-1, 2)],
                    'inner_bottom_right': [(1, 1), (1, 0), (0, 1), (2, 1), (1, 2)]
                }
                
                # For each corner type, look for pieces near the floor tile
                for corner_type, offsets in corner_search_offsets.items():
                    best_candidate = None
                    best_score = 0
                    
                    print(f"      Looking for {corner_type} corner near floor tile at ({floor_x}, {floor_y})")
                    
                    for dx, dy in offsets:
                        tile_x = floor_x + dx
                        tile_y = floor_y + dy
                        
                        if tile_x < 0 or tile_y < 0 or tile_x >= tiles_per_row or tile_y >= tiles_per_col:
                            continue
                        ...
                        try:
                            ...
                            # Score this tile as a potential corner piece that matches our floor
                            score = score_floor_matching_corner(test_surface, corner_type, floor_tile)
                            
                            if score > best_score:
                                best_score = score
                                best_candidate = (tile_x, tile_y, test_surface.copy(), score)
                                print(f"        Found better {corner_type} candidate at ({tile_x}, {tile_y}) [score: {score:.1f}]")
                                
                        except (pygame.error, ValueError):
                            continue
                    
                    # If we found a good candidate, use it
                    if best_candidate and best_score > 5:  # Only use corners with decent scores
                        tile_x, tile_y, surface, score = best_candidate
                        corner_tile = pygame.transform.scale(surface, (tile_size, tile_size))
                        corner_tiles[corner_type] = corner_tile
                        print(f"      ✓ Floor-matching corner {corner_type} from position ({tile_x}, {tile_y}) [score: {score:.1f}]")
                    else:
                        print(f"      ✗ No good floor-matching {corner_type} corner found (best score: {best_score:.1f})")
            else:
                print(f"      ✗ Could not locate floor tile position in tileset - using fallback method")
                # Fallback to looking at common corner positions if we can't find the floor tile
                corner_fallback_positions = {
                    'inner_top_left': [(1, 1), (2, 2), (0, 1), (1, 0)],
                    'inner_top_right': [(tiles_per_row-2, 1), (tiles_per_row-3, 2), (tiles_per_row-1, 1)],
                    'inner_bottom_left': [(1, tiles_per_col-2), (0, tiles_per_col-3), (1, tiles_per_col-1)],
                    'inner_bottom_right': [(tiles_per_row-2, tiles_per_col-2), (tiles_per_row-1, tiles_per_col-1)]
                }
                
                for corner_type, positions in corner_fallback_positions.items():
                    best_candidate = None
                    best_score = 0
                    ...
                    if best_candidate:
                        ...
                        corner_tile = pygame.transform.scale(surface, (tile_size, tile_size))
                        corner_tiles[corner_type] = corner_tile
                        print(f"      ✓ Fallback corner {corner_type} from position ({tile_x}, {tile_y}) [score: {score:.1f}]")
                    else:
                        print(f"      ✗ No fallback {corner_type} corner found")
...
class Dungeon:
    ...
    def create_room(self, room):
        """Create a room with rounded corners like in the image provided."""
        # Calculate room dimensions
        room_width = room.x2 - room.x1 - 1  # Interior width
        room_height = room.y2 - room.y1 - 1  # Interior height
        
        # Create the room with rounded corners
        for x in range(room.x1 + 1, room.x2):
            for y in range(room.y1 + 1, room.y2):
                # Calculate relative position within the room
                rel_x = x - room.x1 - 1  # 0 to room_width-1
                rel_y = y - room.y1 - 1  # 0 to room_height-1
                
                # Check if we're in a corner area
                corner_size = min(3, min(room_width // 3, room_height // 3))
                
                # By default, place floor
                place_floor = True
                
                # Only add rounded corners to rooms that are large enough
                if room_width >= 6 and room_height >= 6 and corner_size >= 2:
                    # Top-left corner
                    if rel_x < corner_size and rel_y < corner_size:
                        # Create a more circular corner using distance from corner
                        distance_sq = (rel_x - (corner_size - 1)) ** 2 + (rel_y - (corner_size - 1)) ** 2
                        # Use a curved threshold for smoother corners
                        threshold = (corner_size - 0.7) ** 2
                        if distance_sq > threshold:
                            place_floor = False  # Keep as wall for rounded effect
                    
                    # Top-right corner
                    elif rel_x >= room_width - corner_size and rel_y < corner_size:
                        distance_sq = (rel_x - (room_width - corner_size)) ** 2 + (rel_y - (corner_size - 1)) ** 2
                        threshold = (corner_size - 0.7) ** 2
                        if distance_sq > threshold:
                            place_floor = False
                    
                    # Bottom-left corner
                    elif rel_x < corner_size and rel_y >= room_height - corner_size:
                        distance_sq = (rel_x - (corner_size - 1)) ** 2 + (rel_y - (room_height - corner_size)) ** 2
                        threshold = (corner_size - 0.7) ** 2
                        if distance_sq > threshold:
                            place_floor = False
                    
                    # Bottom-right corner
                    elif rel_x >= room_width - corner_size and rel_y >= room_height - corner_size:
                        distance_sq = (rel_x - (room_width - corner_size)) ** 2 + (rel_y - (room_height - corner_size)) ** 2
                        threshold = (corner_size - 0.7) ** 2
                        if distance_sq > threshold:
                            place_floor = False
                
                # Place floor tile if not in a rounded corner
                if place_floor:
...
class Game:
    ...
    def get_wall_type(self, x, y):
        """Detect what type of wall piece this should be (corner, edge, etc.) for puzzle-piece system.
        Enhanced to work with curved room generation for better tileset matching."""
        if not self.dungeon:
            return None
        
        # Only apply to wall tiles
        if self.dungeon.grid[y][x] != UI["wall"]:
            return None
        
        # Get adjacent tiles in all 8 directions for wall analysis
        up = self.get_tile_at(x, y-1)
        down = self.get_tile_at(x, y+1) 
        left = self.get_tile_at(x-1, y)
        right = self.get_tile_at(x+1, y)
        up_left = self.get_tile_at(x-1, y-1)
        up_right = self.get_tile_at(x+1, y-1)
        down_left = self.get_tile_at(x-1, y+1)
        down_right = self.get_tile_at(x+1, y+1)
        
        # Enhanced curve detection for rounded room corners
        # Count floor tiles in each direction to understand the room shape
        floor_neighbors = [up, down, left, right].count(UI["floor"])
        diagonal_floors = [up_left, up_right, down_left, down_right].count(UI["floor"])
        
        # ENHANCED INNER CORNERS (curve inward toward room interior)
        # These work better with the rounded room generation
        
        # Top-left inner corner: Enhanced detection for curved rooms
        if (up == UI["wall"] and left == UI["wall"] and 
            down == UI["floor"] and right == UI["floor"]):
            # Additional validation: check if we're at a true corner curve
            if down_right == UI["floor"]:
                return "corner_inner_top_left"
        
        # Top-right inner corner: Enhanced detection 
        if (up == UI["wall"] and right == UI["wall"] and 
            down == UI["floor"] and left == UI["floor"]):
            if down_left == UI["floor"]:
                return "corner_inner_top_right"
        
        # Bottom-left inner corner: Enhanced detection
        if (down == UI["wall"] and left == UI["wall"] and 
            up == UI["floor"] and right == UI["floor"]):
            if up_right == UI["floor"]:
                return "corner_inner_bottom_left"
        
        # Bottom-right inner corner: Enhanced detection
        if (down == UI["wall"] and right == UI["wall"] and 
            up == UI["floor"] and left == UI["floor"]):
            if up_left == UI["floor"]:
                return "corner_inner_bottom_right"
        
        # CURVED WALL DETECTION for rounded room edges
        # These detect the subtle curves created by the rounded room generation
        
        # Curved top-left transition (for rounded corner walls)
        if (up == UI["wall"] and left == UI["wall"] and 
            down == UI["floor"] and right == UI["wall"] and
            down_right == UI["floor"]):
            return "corner_inner_top_left"
        
        # Curved top-right transition
        if (up == UI["wall"] and right == UI["wall"] and 
            down == UI["floor"] and left == UI["wall"] and
            down_left == UI["floor"]):
            return "corner_inner_top_right"
        
        # Curved bottom-left transition  
        if (down == UI["wall"] and left == UI["wall"] and 
            up == UI["floor"] and right == UI["wall"] and
            up_right == UI["floor"]):
            return "corner_inner_bottom_left"
        
        # Curved bottom-right transition
        if (down == UI["wall"] and right == UI["wall"] and 
            up == UI["floor"] and left == UI["wall"] and
            up_left == UI["floor"]):
            return "corner_inner_bottom_right"
        
        # SUBTLE CURVE DETECTION for gradual transitions
        # These catch the intermediate pieces in rounded corners
        
        # Top curve detection (gradual transition from vertical to horizontal)
        if (up == UI["wall"] and down == UI["floor"] and 
            floor_neighbors >= 1 and diagonal_floors >= 1):
            # Check if we're part of a curved top edge
            if ((left == UI["wall"] and up_left == UI["wall"] and down_left == UI["floor"]) or
                (right == UI["wall"] and up_right == UI["wall"] and down_right == UI["floor"])):
                return "corner_inner_top_left" if left == UI["wall"] else "corner_inner_top_right"
        
        # Bottom curve detection
        if (down == UI["wall"] and up == UI["floor"] and 
            floor_neighbors >= 1 and diagonal_floors >= 1):
            if ((left == UI["wall"] and down_left == UI["wall"] and up_left == UI["floor"]) or
                (right == UI["wall"] and down_right == UI["wall"] and up_right == UI["floor"])):
                return "corner_inner_bottom_left" if left == UI["wall"] else "corner_inner_bottom_right"
        
        # Left curve detection
        if (left == UI["wall"] and right == UI["floor"] and 
            floor_neighbors >= 1 and diagonal_floors >= 1):
            if ((up == UI["wall"] and up_left == UI["wall"] and up_right == UI["floor"]) or
                (down == UI["wall"] and down_left == UI["wall"] and down_right == UI["floor"])):
                return "corner_inner_top_left" if up == UI["wall"] else "corner_inner_bottom_left"
        
        # Right curve detection  
        if (right == UI["wall"] and left == UI["floor"] and 
            floor_neighbors >= 1 and diagonal_floors >= 1):
            if ((up == UI["wall"] and up_right == UI["wall"] and up_left == UI["floor"]) or
                (down == UI["wall"] and down_right == UI["wall"] and down_left == UI["floor"])):
                return "corner_inner_top_right" if up == UI["wall"] else "corner_inner_bottom_right"
        
        # OUTER CORNERS (for complex room shapes)
        # These handle convex corners that might occur in connected rooms
        
        # Top-left outer corner
        if (up == UI["wall"] and left == UI["wall"] and up_left == UI["wall"] and
            down == UI["floor"] and right == UI["floor"] and down_right == UI["floor"]):
            return "corner_outer_top_left"
        
        # Top-right outer corner  
        if (up == UI["wall"] and right == UI["wall"] and up_right == UI["wall"] and
            down == UI["floor"] and left == UI["floor"] and down_left == UI["floor"]):
            return "corner_outer_top_right"
        
        # Bottom-left outer corner
        if (down == UI["wall"] and left == UI["wall"] and down_left == UI["wall"] and
            up == UI["floor"] and right == UI["floor"] and up_right == UI["floor"]):
            return "corner_outer_bottom_left"
        
        # Bottom-right outer corner
        if (down == UI["wall"] and right == UI["wall"] and down_right == UI["wall"] and
            up == UI["floor"] and left == UI["floor"] and up_left == UI["floor"]):
            return "corner_outer_bottom_right"
        
        # STRAIGHT EDGE PIECES (for non-curved sections)
        # Only use these when we clearly have straight walls, not curves
        
        # Horizontal edges - only if we're clearly in a straight section
        if (up == UI["wall"] and down == UI["floor"] and
            left == UI["wall"] and right == UI["wall"] and
            floor_neighbors == 1 and diagonal_floors <= 1):
            return "edge_horizontal_top"
            
        if (up == UI["floor"] and down == UI["wall"] and
            left == UI["wall"] and right == UI["wall"] and
            floor_neighbors == 1 and diagonal_floors <= 1):
            return "edge_horizontal_bottom"
        
        # Vertical edges - only if clearly straight
        if (left == UI["wall"] and right == UI["floor"] and
            up == UI["wall"] and down == UI["wall"] and
            floor_neighbors == 1 and diagonal_floors <= 1):
            return "edge_vertical_left"
            
        if (left == UI["floor"] and right == UI["wall"] and
            up == UI["wall"] and down == UI["wall"] and
            floor_neighbors == 1 and diagonal_floors <= 1):
            return "edge_vertical_right"
        
        # SIMPLE EDGE DETECTION for room boundaries
        # These catch basic edges when curves aren't detected
        
        if down == UI["floor"] and up != UI["floor"] and floor_neighbors == 1:
            return "edge_top"
        if up == UI["floor"] and down != UI["floor"] and floor_neighbors == 1:
            return "edge_bottom"
        if right == UI["floor"] and left != UI["floor"] and floor_neighbors == 1:
            return "edge_left"
        if left == UI["floor"] and right != UI["floor"] and floor_neighbors == 1:
            return "edge_right"
        
        # Default: solid wall piece (interior wall)
        return "wall_solid"
        if (up == UI["wall"] and right == UI["wall"] and 
            down == UI["floor"] and left == UI["floor"]):
            return "corner_inner_top_right"
        
        # Bottom-left inner corner: walls below and left, floor/open space to top-right
        if (down == UI["wall"] and left == UI["wall"] and 
            up == UI["floor"] and right == UI["floor"]):
            return "corner_inner_bottom_left"
        
        # Bottom-right inner corner: walls below and right, floor/open space to top-left
        if (down == UI["wall"] and right == UI["wall"] and 
            up == UI["floor"] and left == UI["floor"]):
            return "corner_inner_bottom_right"
        
        # ADDITIONAL CORNER DETECTION for room boundaries
        # These catch corners where one side might be a boundary instead of explicit floor
        
        # Top-left corner at room boundary
        if (up == UI["wall"] and left == UI["wall"] and 
            (down == UI["floor"] or right == UI["floor"])):
            # Check if this creates an inward corner
            if down_right == UI["floor"]:
                return "corner_inner_top_left"
        
        # Top-right corner at room boundary  
        if (up == UI["wall"] and right == UI["wall"] and 
            (down == UI["floor"] or left == UI["floor"])):
            if down_left == UI["floor"]:
                return "corner_inner_top_right"
        
        # Bottom-left corner at room boundary
        if (down == UI["wall"] and left == UI["wall"] and 
            (up == UI["floor"] or right == UI["floor"])):
            if up_right == UI["floor"]:
                return "corner_inner_bottom_left"
        
        # Bottom-right corner at room boundary
        if (down == UI["wall"] and right == UI["wall"] and 
            (up == UI["floor"] or left == UI["floor"])):
            if up_left == UI["floor"]:
                return "corner_inner_bottom_right"
        
        # OUTER CORNERS (for complex room shapes)
        
        # Top-left outer corner: walls form L-shape with diagonal wall
        if (up == UI["wall"] and left == UI["wall"] and up_left == UI["wall"] and
            down == UI["floor"] and right == UI["floor"] and down_right == UI["floor"]):
            return "corner_outer_top_left"
        ...
        # Only apply these if we're not at a corner
        
        # Horizontal top edge: wall above, floor below, walls on both sides AND not a corner
        if (up == UI["wall"] and down == UI["floor"] and
            left == UI["wall"] and right == UI["wall"]):
            # Make sure this isn't actually a corner by checking diagonals
            if not ((up_left == UI["wall"] and down_left == UI["floor"] and down_right == UI["floor"]) or
                   (up_right == UI["wall"] and down_left == UI["floor"] and down_right == UI["floor"])):
                return "edge_horizontal_top"
        
        # Horizontal bottom edge: floor above, wall below, walls on both sides AND not a corner
        if (up == UI["floor"] and down == UI["wall"] and
            left == UI["wall"] and right == UI["wall"]):
            # Make sure this isn't actually a corner
            if not ((down_left == UI["wall"] and up_left == UI["floor"] and up_right == UI["floor"]) or
                   (down_right == UI["wall"] and up_left == UI["floor"] and up_right == UI["floor"])):
                return "edge_horizontal_bottom"
        
        # Vertical left edge: wall to left, floor to right, walls above and below AND not a corner
        if (left == UI["wall"] and right == UI["floor"] and
            up == UI["wall"] and down == UI["wall"]):
            # Make sure this isn't actually a corner
            if not ((up_left == UI["wall"] and up_right == UI["floor"] and down_right == UI["floor"]) or
                   (down_left == UI["wall"] and up_right == UI["floor"] and down_right == UI["floor"])):
                return "edge_vertical_left"
        
        # Vertical right edge: floor to left, wall to right, walls above and below AND not a corner
        if (left == UI["floor"] and right == UI["wall"] and
            up == UI["wall"] and down == UI["wall"]):
            # Make sure this isn't actually a corner
            if not ((up_right == UI["wall"] and up_left == UI["floor"] and down_left == UI["floor"]) or
                   (down_right == UI["wall"] and up_left == UI["floor"] and down_left == UI["floor"])):
                return "edge_vertical_right"
        
        # SIMPLE DIRECTIONAL EDGES (for thinner walls and room boundaries)
        # These are simpler checks for when we have basic edge situations
        
        # Top edge (floor below, wall/boundary above) - not a corner
        if (down == UI["floor"] and up != UI["floor"] and 
            not ((left == UI["wall"] and up == UI["wall"]) or 
                 (right == UI["wall"] and up == UI["wall"]))):
            return "edge_top"
        
        # Bottom edge (floor above, wall/boundary below) - not a corner
        if (up == UI["floor"] and down != UI["floor"] and
            not ((left == UI["wall"] and down == UI["wall"]) or 
                 (right == UI["wall"] and down == UI["wall"]))):
            return "edge_bottom"
        
        # Left edge (floor to right, wall/boundary to left) - not a corner
        if (right == UI["floor"] and left != UI["floor"] and
            not ((up == UI["wall"] and left == UI["wall"]) or 
                 (down == UI["wall"] and left == UI["wall"]))):
            return "edge_left"
        
        # Right edge (floor to left, wall/boundary to right) - not a corner
        if (left == UI["floor"] and right != UI["floor"] and
            not ((up == UI["wall"] and right == UI["wall"]) or 
                 (down == UI["wall"] and right == UI["wall"]))):
            return "edge_right"
        
        # Default: solid wall piece (interior wall, no edges)
        return "wall_solid"

    def get_corner_type(self, x, y):
        """Detect if a floor tile should use a corner piece using Undertale bg_firstroom.png logic."""
        if not self.dungeon:
            return None
        ...
        # Get adjacent tiles in all 8 directions (like Undertale room analysis)
        up = self.get_tile_at(x, y-1)
        down = self.get_tile_at(x, y+1) 
        left = self.get_tile_at(x-1, y)
        right = self.get_tile_at(x+1, y)
        ...
        # Undertale bg_firstroom.png shows that rounded corners appear where:
        # 1. Floor meets walls in corner patterns (for inner corners)
        # 2. Multiple walls create concave shapes that need rounding
        # 3. Room shapes transition smoothly rather than having harsh 90-degree angles
        
        # Inner corner detection (like bg_firstroom.png corner shapes)
        # These create the smooth, rounded appearance seen in Undertale rooms
        
        # Top-left inner corner: walls above and left, but floor in other directions
        # This matches the rounded corners visible in bg_firstroom.png
        if (up == UI["wall"] and left == UI["wall"] and 
            down == UI["floor"] and right == UI["floor"]):
            # Additional check: ensure we're not in a thin corridor
            if down_right == UI["floor"]:
                return "inner_top_left"
        
        # Top-right inner corner: walls above and right, floor elsewhere
        if (up == UI["wall"] and right == UI["wall"] and 
            down == UI["floor"] and left == UI["floor"]):
            if down_left == UI["floor"]:
                return "inner_top_right"
        
        # Bottom-left inner corner: walls below and left, floor elsewhere
        if (down == UI["wall"] and left == UI["wall"] and 
            up == UI["floor"] and right == UI["floor"]):
            if up_right == UI["floor"]:
                return "inner_bottom_left"
        
        # Bottom-right inner corner: walls below and right, floor elsewhere
        if (down == UI["wall"] and right == UI["wall"] and 
            up == UI["floor"] and left == UI["floor"]):
            if up_left == UI["floor"]:
                return "inner_bottom_right"
                
        # Additional Undertale-style corner detection for more complex room shapes
        # Based on bg_firstroom.png's natural corner transitions
        
        # Outer corners (less common but create nice transitions)
        # These help create the organic feel seen in Undertale backgrounds
        
        # Top-left outer corner: floor surrounded by walls in L-shape
        if (up == UI["wall"] and left == UI["wall"] and up_left == UI["wall"] and
            down == UI["floor"] and right == UI["floor"] and 
            down_right == UI["floor"]):
            return "outer_top_left"
            
        # Top-right outer corner: floor with walls forming right L-shape
        if (up == UI["wall"] and right == UI["wall"] and up_right == UI["wall"] and
            down == UI["floor"] and left == UI["floor"] and 
            down_left == UI["floor"]):
            return "outer_top_right"
            
        # Bottom-left outer corner
        if (down == UI["wall"] and left == UI["wall"] and down_left == UI["wall"] and
            up == UI["floor"] and right == UI["floor"] and 
            up_right == UI["floor"]):
            return "outer_bottom_left"
            
        # Bottom-right outer corner
        if (down == UI["wall"] and right == UI["wall"] and down_right == UI["wall"] and
            up == UI["floor"] and left == UI["floor"] and 
            up_left == UI["floor"]):
                ...
                if is_explored:
                    if game_settings['use_emojis']:
                        ...
                        if tile_type == UI["wall"]:
                            # Puzzle-piece wall system: detect wall type and use appropriate sprite
                            wall_type = self.get_wall_type(world_x, world_y)
                            wall_sprite_used = False
                            
                            if wall_type:
                                # Try to get wall piece from current level's tileset
                                if ("level_sprites" in sprites and 
                                    self.dungeon_level in sprites["level_sprites"] and
                                    "walls" in sprites["level_sprites"][self.dungeon_level] and
                                    wall_type in sprites["level_sprites"][self.dungeon_level]["walls"]):
                                    
                                    wall_tile = sprites["level_sprites"][self.dungeon_level]["walls"][wall_type]
                                    sprite = wall_tile.copy()
                                    if not is_visible:
                                        sprite.set_alpha(128)
                                    screen.blit(sprite, (screen_x, screen_y))
                                    wall_sprite_used = True
                                    sprite_drawn = True
                            
                            # If no specific wall piece was used, draw generic wall
                            if not wall_sprite_used:
                                sprite_key = f"wall_{game_settings['wall_sprite']}"
                                if sprite_key in sprites:
                            ...
                            # Floor tiles use consistent interior texture (no corners on floor anymore)
                            # All floor tiles should look the same for the puzzle-piece effect
                            
                            # Use level-specific floor tile if available
                            floor_sprite_used = False
                            if ("level_sprites" in sprites and 
                                self.dungeon_level in sprites["level_sprites"] and
                                "floor" in sprites["level_sprites"][self.dungeon_level]):
                                
                                floor_tile = sprites["level_sprites"][self.dungeon_level]["floor"]
                                sprite = floor_tile.copy()
                                if not is_visible:
                                    sprite.set_alpha(128)  # Make dimmer if not visible
                                screen.blit(sprite, (screen_x, screen_y))
                                floor_sprite_used = True
                                sprite_drawn = True
                            
                            # Fallback to settings-based floor if level-specific not available
                            if not floor_sprite_used:
                                sprite_key = f"floor_{game_settings['floor_sprite']}"
                                if sprite_key in sprites:
                                    sprite = sprites[sprite_key].copy()
                                    if not is_visible:
